...
//				Debug.Log ("END MENU CALLED");
		if (!endCalled) {
			...
			newHighScore = checkForHighScore ();
	...
	{
		if (totalCaughtCount > PlayerPrefs.GetInt (Application.loadedLevelName + "count", 0)) {
			PlayerPrefs.SetInt (Application.loadedLevelName + "count", totalCaughtCount);
			return true;
		} else {
			return false;
		}
